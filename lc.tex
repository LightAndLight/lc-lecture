...
\begin{frame}
  ...
  \begin{itemize}
    \item<6-10> $ x[x\backslash a] \leadsto a $
    \item<7-10> $ y[x\backslash a] \leadsto y \;\;\; (\text{given} \; y \neq x) $
    \item<8-10> $ (m \; n)[x\backslash a] \leadsto m[x\backslash a] \; n[x\backslash a]$
    \item<9-10> $ (\lambda x. \; e)[x\backslash a] \leadsto \lambda x. \; e $
    \item<10> $ (\lambda y. \; e)[x\backslash a] \leadsto \lambda y. \; e[x\backslash a] \;\;\; (\text{given} \; y \neq x) $
  \end{itemize}
  }
  ...
  \begin{itemize}
    \item<12> $ (\lambda x. e) \; a \leadsto e[x\backslash a] $ 
  \end{itemize}
  }
...
\begin{frame}
  ...
  { \Huge \[ (\lambda f. \; f \; x) \; (\lambda b. \; b) \leadsto x \] }
  \[
...
\begin{frame}
  ...
  \only<1>{\; ?}
  \only<2->{
    \lambda b. \; \only<2>{?} \only<3->{ b \; \only<3>{?} \only<4->{ \texttt{true} } \; \only<3,4>{?} \only<5->{ \texttt{false} } }
  }
  \]
...
\begin{frame}
  ...
  \only<2->{
    \lambda a. \;
    \only<2>{?}
    \only<3->{ \lambda b. \; \only<3>{?} \only<4->{ a \; \only<4>{?} \only<5->{ \texttt{false} } \; \only<4,5>{?} \only<6->{ b } } }
  }
  \]
...
\begin{frame}
  ...
  \begin{center}
  $ ifThenElse \; \texttt{true} \; a \; b $ should be $a$ \\
  $ ifThenElse \; \texttt{false} \; a \; b $ should be $b$
  \end{center}
  ...
  \only<2->{
    ...
    \only<3->{
      ...
      \only<4->{
        ...
        \only<4>{?}
        \only<5->{
          b \; \only<5>{?} \only<6->{y} \; \only<5,6>{?} \only<7->{x}
        }
      }
    }
  }
...
\begin{frame}
  ...
  \begin{center}
  $ \langle a, b \rangle = \lambda f. \; f \; a \; b $ \\ ~ \\
  $ \texttt{fst} \; \langle x, y \rangle $ should be $x$ \\
  \end{center}
  \[
  \texttt{fst} =
    \;
...
\begin{frame}
  ...
  \[ \only<3->{\greyafter{3}{(\lambda f. \; \lambda x. \; f \; (n \; f \; x))[n\backslash  \texttt{zero}]}} \]
  ...
  \onslide<2->{
    ...
    \only<3->{
      ...
      \only<4->{
        ...
        \only<4>{?}
        \only<5->{
          m \; \only<5>{?} \only<6->{f} \; \only<5,6>{?} \only<7->{(n \; f \; x)}
        }
      }
    }
  }
